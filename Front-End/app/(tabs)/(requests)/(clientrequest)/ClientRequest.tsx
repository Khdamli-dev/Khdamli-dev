...
const PublicRequest = () => {
  ...
  const deleteRequest = async (requestId: number) => {
    try {
      ...
      // Check if response exists and has data (don't assume success property exists)
      console.log("Delete response:", response.data);
      // Assume successful delete if we get here without error
        ...
        <View className="flex-row justify-between items-center">
          <View className="flex-row items-center flex-1">
            <View className="flex-1">
              <Text className="font-medium">{timeAgo(item.sent_date)}</Text>
          ...
          <View className="flex-row justify-between items-center">
            <View className="flex-1">
              <View className="flex-row justify-between">
                <Text className="text-gray-600">{timeAgo(item.sent_date)}</Text>
                <View className="flex-row items-center">
                  {getStatusIcon(item.status)}
                  <Text className="ml-1 text-gray-600 text-sm capitalize">
                    {item.status}
  ...
  const renderWorkerRequest = (item: WorkerPublicRequest) => {
    ...
    return (
      <View className="bg-white mt-3 mb-4 p-4 rounded-lg shadow">
        ...
        <View className="mb-4">
          <Text className="text-base mb-2">
            <Text className="font-bold text-black">Request Date: </Text>
            <Text className="text-green-500">
              {formatDateTime(item.post_date)}
            </Text>
          </Text>
          <Text className="text-base mb-2">
            <Text className="font-bold text-black">Work Address: </Text>
            <Text className="text-green-500">
              {item.location?.city}, {item.location?.region},{" "}
              {item.location?.country}
            </Text>
        ...
        >
          <Text className="text-base text-white font-medium">Edit Comment</Text>
        </TouchableOpacity>
      </View>
    );
  };
